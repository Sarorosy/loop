...
export default function ManageBucket({ onClose }) {
  ...
  const fetchBuckets = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://loopback-skci.onrender.com/api/helper/allbuckets", {
        method: "GET",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://loopback-skci.onrender.com/api/helper/bucket/delete/${selectedBucket?.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        }
