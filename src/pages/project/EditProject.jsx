...
export default function EditProject({ onClose, projectData, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://loopback-skci.onrender.com/api/helper/project/update/${projectData.id}`, {
        method: "PUT",
    ...
    >
      ...
      <form  className="p-4 space-y-4">
        ...
        <div className="flex justify-end">
          ...
          >
            {loading ? "Updating..." : "Update Project"}<ChevronsRightIcon size={11} className="" />
          </button>
        </div>
      </form>
    </motion.div>
