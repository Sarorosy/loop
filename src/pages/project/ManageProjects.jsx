...
export default function ManageProjects({ onClose }) {
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://loopback-n3to.onrender.com/api/helper/allprojects", {
        method: "GET",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://loopback-n3to.onrender.com/api/helper/project/delete/${selectedProject.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        }
